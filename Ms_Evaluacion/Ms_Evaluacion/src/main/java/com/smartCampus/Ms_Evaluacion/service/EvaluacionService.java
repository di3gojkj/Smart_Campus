...
public class EvaluacionService {
    ...
    public List<EvaluacionResponseDTO> buscarPorTipo(Long tipoId) {
        ...
        if (lista.isEmpty()) {
            throw new EvaluacionNotFoundException(tipoId);
        }
    ...
    public EvaluacionResponseDTO crear(EvaluacionRequestDTO dto) {
        if (evaluacionRepository.existsByNombreIgnoreCase(dto.getNombre())) {
            throw new EvaluacionConflictException("Ya existe una evaluacion con ese nombre");
        }

        TipoEvaluacion tipoEval = tipoEvaluacionRepository.findById(dto.getIdTipoEval())
                .orElseThrow(() -> new TipoEvaluacionNotFoundException(dto.getIdTipoEval()));
        ...
        logger.info("Evaluacion creada correctamente con ID: {}", guardada.getIdEvaluacion());
    ...
    public EvaluacionResponseDTO actualizar(Long id, EvaluacionRequestDTO dto) {
        ...
        if (evaluacionRepository.existsByNameAndTipoExcludingId(dto.getNombre(), nuevoTipo.getIdTipoEval(), id)) {
            throw new EvaluacionConflictException("Ya existe otra evaluacion con ese nombre en este tipo");
        }
        ...
        logger.info("Evaluacion ID: {} actualizada correctamente", id);
    ...
    public void eliminar(Long id) {
        ...
        logger.info("Evaluacion ID: {} eliminada correctamente", id);
    }
